...
```mermaid
flowchart TB
  UI["Next.js"] --> FA["FastAPI"]
  FA --> SVC["services/"]
  SVC --> API["api/ clients"]
  SVC --> MOD["models/"]
  SVC --> UTIL["utils/ EPSG:27700"]
  FA --> CORE["core/"]
  API --> EXT1["Address API"]
  API --> EXT2["GeoServer WFS"]
```
...
```mermaid
flowchart LR
  WFS["WFS features"] --> PARSE["Parse SP_GEOMETRY"]
  PARSE --> BNG["EPSG:27700"]
  BNG --> DIST["Euclidean distance"]
  DIST --> NEAR["Nearest within ~100m"]
```
...
```mermaid
flowchart LR
  ALL["All grit bins"] --> SORT["Sort by distance ascending"]
  SORT --> TOP["Return top 5"]
```
